...
1. Train
   ...
   ```bash
    ...
    #The parameters -t : train, -z : changing_the_reward_function_to_high_jump_reward (Bool), -w : wall existence for path (Bool), -ws : wall_size for path (Float)
    #-tw : train_wall (Bool) -tws : modify the wall size(Float)
